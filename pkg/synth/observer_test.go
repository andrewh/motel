...
func TestObserverCalledPerSpan(t *testing.T) {
	...
	engine.walkTrace(context.Background(), topo.Roots[0], time.Now(), 0, nil, &Stats{}, new(int), DefaultMaxSpansPerTrace)
...
func TestObserverNotCalledWhenNone(t *testing.T) {
	...
	engine.walkTrace(context.Background(), engine.Topology.Roots[0], time.Now(), 0, nil, &Stats{}, new(int), DefaultMaxSpansPerTrace)
