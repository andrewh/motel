...
- Replay mode: `motel import --record <file>` writes a newline-delimited
  ...
  `mode: replay` config (with `recording: <file>`) re-emits those traces exactly
  via `motel run`. Timestamps are shifted to run time by default, or preserved
  verbatim with `--verbatim`. Non-realtime only for now; span kind is derived
  from tree position. (#209)
