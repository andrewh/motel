...
Alternates between a base rate and periodic high-rate bursts. The default burst is 5x for 30 seconds every 5 minutes.
...
Poisson uses a constant mean rate with randomised inter-arrival times. The rate curve is identical to uniform — the randomness affects individual trace timing, not aggregate throughput. Use `motel run` rather than preview to observe the difference.
